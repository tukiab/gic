 ...
 */
class ListaVisitas implements IIterador{
	/**
	...
	public function buscar($filtros){
		...
		(isset($filtros['id']))?$filtro.=" AND visitas.id = '".$filtros['id']."' ":null;

		(isset($filtros['fecha_desde']))?$filtro.=" AND visitas.fecha >= '".$filtros['fecha_desde']."' ":null;
		(isset($filtros['fecha_hasta']))?$filtro.=" AND visitas.fecha <= '".$filtros['fecha_hasta']."' ":null;

		if(isset($filtros['gestor']) &&($filtros['gestor'] != ''))
			$filtro .= " AND visitas.fk_usuario = '".$filtros['gestor']."'";
		
		if(isset($filtros['id_proyecto']))
			$filtro .= " AND visitas.fk_proyecto = '".$filtros['fk_proyecto']."'";
		
		if(isset($filtros['nombre_proyecto']))
			$join .= "INNER JOIN proyectos ON visitas.fk_proyecto = proyectos.id AND proyectos.nombre LIKE '".$filtros['nombre_proyecto']."' ";

		if(isset($datos['nombre_cliente'])){
			if(!$join)
				$join .= " INNER JOIN proyectos ON visitas.fk_proyecto = proyectos.id ";
