...
class TipoDeFormaDePago{
	...
	/**
	 * Identificador de el TipoDeFormaDePago. Coincide con el id de la BBDD.
	 * @var integer
	 */
	private $id;
	...
	/**
	 * Constructor de la clase TipoDeFormaDePago.
	 *
	 * Si recibe un identificador válido, se carga el TipoDeFormaDePago de la BBDD mediante el método cargar(), en caso contrario crea un objeto
	 * vacío, permitiendo insertar una TipoDeFormaDePago nuevo mediante el método crear().
	 * @see cargar()
	 * @see crear()
	 *
	 * @param integer $id_grupoEmpresas Id de el TipoDeFormaDePago. Cuando está definido, se carga de la BBDD.
	 */
	...
	/**
	 * Carga todos los datos para el TipoDeFormaDePago en la BBDD.
	 *
	 * Este método es invocado cuando se le pasa un id de TipoDeFormaDePago válido al constructor, y a partir
	...
	private function cargar(){
		if($this->id){
			...
			if(!($result = mysql_query($query)))
				...
				throw new Exception("No se ha encontrado el tipo de forma de pago en la BBDD");
	...
	/**
	 * Crea una nueva TipoDeFormaDePago en la BBDD a partir de un array indexado con todos los campos.
	 *
	 ...
	 *
	 * @see guardar()
	 * @param array $datos Array indexado con todos los atributos para una nueva TipoDeFormaDePago.
	...
	public function crear($datos){
		...
		if($datos['nombre'] == '' || ! isset($datos['nombre']))
			throw new Exception("TipoDeFormaDePago: El nombre es obligatorio.");
	...
	public function set_nombre($nombre){
		...
		if($validar->cadena($nombre)){
			$query = "UPDATE formas_de_pago SET nombre='$nombre' WHERE id='$this->id' ";
			if(!mysql_query($query))
	...
	public function eliminar(){
		$query = "DELETE FROM formas_de_pago WHERE id = '$this->id';";
		mysql_query($query);
	}	
}
?>
