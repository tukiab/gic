...
<!-- RESULTADOS -->
		...
		<?php if($gestor_actual->esAdministrador()){?><!-- <input type="submit" id="exportar" name="exportar" value="<?php echo  _translate("Exportar")?>" /> --><?php }?>
			<table>
						...
						<th style="text-align: center;font-size: x-small;font-weight: normal" nowrap>
							...
							<?php 
								if($var->opt['order_by']=='usuario' && $var->opt['order_by_asc_desc']=='ASC')
									echo  "&uarr;";
								else if($var->opt['order_by']=='usuario' && $var->opt['order_by_asc_desc']=='DESC')
									echo  "&darr;";
							?>
						</th>
						<th style="text-align: center;font-size: x-small;font-weight: normal" nowrap>
							...
							<?php 
								if($var->opt['order_by']=='tipo_accion' && $var->opt['order_by_asc_desc']=='ASC')
									echo  "&uarr;";
								else if($var->opt['order_by']=='tipo_accion' && $var->opt['order_by_asc_desc']=='DESC')
									echo  "&darr;";
							?>
						</th>
						<th style="text-align: center;font-size: x-small;font-weight: normal" nowrap>
				...
				<?php $fila_par=true;
				?>
				<?php while($accion = $var->datos['lista_acciones']->siguiente() ){
					?>
				...
				<?php 
					...
					<tr>	
						<td>
							<?php echo  $var->datos['lista_acciones']->num_Resultados()?>&nbsp;<?php echo  _translate("Resultados")?>
						</td>
						<td colspan="10">
