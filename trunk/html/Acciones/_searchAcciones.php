...
class BusquedaAcciones{
	...
	public function __construct($opciones){
		...
		if($this->opt['eliminar']) $this->eliminarAcciones();
		//Asignamos a una variable el objeto Lista_Acciones
		$this->lista_Acciones =  new ListaAcciones();
		...
		$this->lista_Acciones->buscar($this->opt, $this->datos['page'], $this->datos['paso']);
		//$this->lista_Acciones->buscar($this->opt);
		$total = $this->lista_Acciones->num_Resultados();
	...
	private function eliminarAcciones(){
		foreach($this->opt['seleccionados'] as $idAccion){
	...
	private function obtener_Opciones($opciones){
		...
		@($opciones['id_cliente'])?$this->opt['id_cliente']=$opciones['descripcion']:null;
		@($opciones['tipo_accion'])?$this->opt['tipo_accion']=$opciones['tipo_accion']:null;			
		@($opciones['descripcion'])?$this->opt['descripcion']=$opciones['descripcion']:null;
	 ...
	 */
	 private function obtener_Listas(){
	 	$this->datos['lista_tipos_acciones'] = $this->lista_Acciones->lista_Tipos();
		$this->datos['lista_gestores'] = $this->lista_Acciones->lista_Gestores();
	 }
